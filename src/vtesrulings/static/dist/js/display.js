...
})({"bTuTa":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "58b928829d070254";
"use strict";
...
},{}],"gpcRA":[function(require,module,exports) {
...
(0, _autocompleteJsDefault.default).init();
class UrlState {
    state;
    callback;
    multivalued;
    base_location;
    constructor(callback, multivalued){
        this.state = {};
    ...
    async setup() {
        ...
        if (window.location.search) {
            ...
            this.state = {};
            for (const [k, v] of params)if (this.multivalued.has(k)) this.state[k] = v.split("|");
            else this.state[k] = v;
        } else this.state = {};
    ...
    refresh() {
        ...
        if (Object.keys(this.state).length > 0) {
            let params_map = {};
            for (const [k, v] of Object.entries(this.state))if (this.multivalued.has(k)) params_map[k] = v.join("|");
            else params_map[k] = v;
...
const ERROR_TOAST_DIV = document.getElementById("errorToast");
let ERROR_TOAST = _bootstrap.Toast.getOrCreateInstance(ERROR_TOAST_DIV);
...
function displayError(msg) {
    let body = ERROR_TOAST_DIV.querySelector("div.toast-body");
    body.innerText = msg;
    ERROR_TOAST.show();
}
class EditMode {
    ...
    referencelModal;
    constructor(){
        this.proposalButton = document.getElementById("proposalButton");
        this.proposalForm = document.getElementById("proposalForm");
        this.proposalStart = document.getElementById("proposalStart");
        this.proposalSubmit = document.getElementById("proposalSubmit");
        this.proposalApprove = document.getElementById("proposalApprove");
        this.proposalMeta = document.querySelector('meta[name="proposal"]');
        this.referenceNewButton = document.getElementById("referenceNewButton");
        if (this.proposalMeta) {
            this.proposal = JSON.parse(this.proposalMeta.content);
            this.displayProposal(this.proposal);
        }
        this.proposalModal = new _bootstrap.Modal("#proposalModal");
        this.referencelModal = new _bootstrap.Modal("#referenceModal");
    ...
    displayRulingCard(elem, ruling, mainUid = "") {
        ...
        for (const symbol of ruling.symbols)text = text.replaceAll(symbol.text, `<span class="krcg-icon" contenteditable="false">${symbol.symbol}</span>`);
        ...
        if (edit_mode) {
            ...
            card_text.addEventListener("input", debounce(async ()=>{
        ...
        if (edit_mode) {
            ...
            plus_button.addEventListener("click", ()=>{
                ...
                this.referenceNewButton.addEventListener("click", async (ev)=>{
                    await createAndAddLink(ev, card);
                    this.referencelModal.hide();
                }, {
                    once: true
                });
            });
    ...
    displayProposal(proposal) {
        ...
        const proposalAccBody = document.getElementById("proposalAccBody");
        ...
        for (const mod of proposal.modified){
            ...
            let page = undefined;
            if (mod.type === "group") page = "groups.html";
...
let CURRENT_P = undefined;
let CURRENT_NODE = undefined;
let OFFSET = 0;
function displayEditTools(event) {
    // runs when focusin on one of the rulings <p> element
    CURRENT_P = event.target;
...
async function createAndAddLink(ev, card) {
    try {
        const response = await fetch("http://127.0.0.1:5000/reference", {
            method: "post",
            body: new FormData(ev.target.form)
        });
...
function addRulingReference(card, links_div, data, edit_mode, prepend) {
    ...
    if (prepend) links_div.prepend(link_div);
    else links_div.append(link_div);
}
function insertDisc(clickEvent) {
    const selection = window.getSelection();
    const range = selection.getRangeAt(0);
    const newElement = document.createElement("span");
    newElement.classList.add("krcg-icon");
    newElement.contentEditable = "false";
    newElement.innerText = clickEvent.target.innerText;
...
async function rulingSave(elem) {
    ...
    for (const node of text){
        if (node.nodeType == Node.TEXT_NODE) full_text += node.nodeValue;
        else if (node.nodeType == Node.ELEMENT_NODE && node.nodeName == "SPAN") {
            let span_elem = node;
            if (span_elem.classList.contains("krcg-icon")) full_text += `[${ANKHA_SYMBOLS_REVERSE[span_elem.innerText]}]`;
            else if (span_elem.classList.contains("krcg-card")) full_text += `{${span_elem.innerText}}`;
        }
    }
    for (const reference of elem.querySelectorAll("a.krcg-reference"))full_text += ` [${reference.innerText}]`;
    if (full_text != elem.dataset.KrcgRulingText) {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/ruling/${elem.dataset.KrcgRulingTargetUid}/${elem.dataset.KrcgRulingUid}`, {
                method: "put",
                body: JSON.stringify({
                    text: full_text
                })
    ...
    } else console.log("Ruling unchanged", elem.dataset.KrcgRulingUid);
}
class CardSearch extends EditMode {
    ...
    constructor(){
        ...
        this.cardAdv = document.getElementById("cardAdv");
        this.cardText = document.getElementById("cardText");
        this.rulingsDiv = document.getElementById("rulingsDiv");
        this.rulingsList = document.getElementById("rulingsList");
        this.backrefTitle = document.getElementById("backrefTitle");
        this.backrefList = document.getElementById("backrefList");
        this.autocomplete = new (0, _autocompleteJsDefault.default)(this.cardSearchInput, {
            "onSelectItem": (item)=>this.state.reset({
                    uid: item.value
                })
        });
        this.state = new UrlState(async (data)=>await this.displayCard(data));
    }
    ...
    processCardData(data) {
        ...
        if (data.group) {
            ...
            if (data.group === "ANY") this.cardGroup.innerText = data.group;
            else this.cardGroup.innerText = "G" + data.group;
        } else this.cardGroup.classList.add("invisible");
        if (data.advanced) this.cardAdv.classList.remove("invisible");
        else this.cardAdv.classList.add("invisible");
        let text = data.text;
        for (const symbol of data.text_symbols)text = text.replaceAll(symbol.text, `<span class="krcg-icon">${symbol.symbol}</span>`);
        ...
        for (const ruling of data.rulings)this.displayRulingCard(this.rulingsList, ruling, data.uid);
        if (data.backrefs && data.backrefs.length > 0) this.backrefTitle.classList.remove("invisible");
        else this.backrefTitle.classList.add("invisible");
        this.backrefList.innerHTML = "";
        for (const card of data.backrefs){
            let div = document.createElement("a");
            div.href = this.state.base_location + "?" + new URLSearchParams({
                uid: card.uid
            }).toString();
...
class GroupPage extends EditMode {
    ...
    constructor(){
        ...
        this.groupDisplay = document.getElementById("groupDisplay");
    }
    displayGroup() {
        if (!this.groupDisplay) return;
        const rulings = JSON.parse(this.groupDisplay.dataset.rulings);
        for (let ruling of rulings)this.displayRulingCard(this.groupDisplay, ruling, this.groupDisplay.dataset.uid);
        const proposalMeta = document.querySelector('meta[name="proposal"]');
        if (proposalMeta) {
            const proposal = JSON.parse(this.proposalMeta.content);
            const groupName = document.getElementById("groupName");
            groupName.contentEditable = "true";
            groupName.addEventListener("input", debounce(async ()=>{
                await groupSave(this.groupDisplay);
            }));
        }
    }
}
async function groupSave(groupDisplay) {
    ...
    for (const card of cards){
        const card_a = card;
        let prefix = "";
        const symbol_span = card_a.querySelector("span.krcg-icon");
        for (const char of symbol_span.innerText)prefix += `[${ANKHA_SYMBOLS_REVERSE[char]}]`;
        body.cards[card_a.dataset.uid] = prefix;
    }
    console.log("Updating group", body);
    try {
        const response = await fetch(`http://127.0.0.1:5000/group/${groupDisplay.dataset.uid}`, {
            method: "put",
            body: JSON.stringify(body)
        });
        if (!response.ok) throw new Error((await response.json())[0]);
        const groupsList = document.getElementById("groupsList");
        const current = groupsList.querySelector("a.active");
        ...
        if (current.dataset.uid === groupDisplay.dataset.uid) {
            ...
            const counter = current.querySelector("span.badge");
...
function addCardEvents(pelem) {
    for (let elem of pelem.children)if (elem.classList.contains("krcg-card")) {
...
const ANKHA_SYMBOLS_REVERSE = Object.fromEntries(Object.entries(ANKHA_SYMBOLS).map(([k, v])=>[
        v,
        k
    ]));
let EDIT_CONTROLS = document.createElement("div");
...
new _bootstrap.Dropdown(button);
let CARD_SEARCH = document.createElement("form");
...
new (0, _autocompleteJsDefault.default)(input, {
    "onSelectItem": (item)=>insertCard(item)
});

},{"bootstrap":"h36JB","bootstrap5-autocomplete/autocomplete.js":"hiuLP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"h36JB":[function(require,module,exports) {
/*!
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hiuLP":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["bTuTa","gpcRA"], "gpcRA", "parcelRequire94c2")
